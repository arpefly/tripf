...
Проблема учёта и разделения групповых расходов имеет несколько важных аспектов. Во-первых, в процессе совместного мероприятия возникает множество различных трат: бронирование жилья, покупка билетов, оплата питания, расходы на развлечения и транспорт. Во-вторых, часто один или несколько участников оплачивают расходы за всю группу, что требует последующих взаиморасчётов. В-третьих, необходимо обеспечить прозрачность и справедливость распределения расходов с учётом того, что разные участники могут иметь различный уровень участия в тех или иных тратах.

Традиционные методы ведения учёта групповых расходов включают использование бумажных блокнотов, текстовых сообщений в мессенджерах или электронных таблиц. Однако такие подходы имеют существенные недостатки:
...
Существующие решения для разделения расходов, такие как Splitwise, 
Tricount и Settle Up, решают эти проблемы, хоть и имеют свои недостатки:  зависимость от постоянного интернет-соединения, хранение персональных финансовых данных на внешних серверах, наличие платных функций для базовых возможностей, а также сложность интерфейса в некоторых приложениях.

\textbf{Актуальность работы} определяется необходимостью создания решения, которое сочетало бы простоту использования, локальное хранение данных, возможность работы в условиях ограниченного доступа к сети и полный контроль пользователей над своей информацией.

\textbf{Цель данной работы} -- разработка веб-приложения для управления групповыми расходами, которое обеспечивает автоматизацию расчётов, прозрачность финансовых операций и возможность работы в оффлайн-режиме.

\textbf{Задачи:}

\begin{enumerate}
    \item Разработать архитектуру веб-приложения на основе современных технологий
    \item Спроектировать структуру базы данных для хранения информации о пользователях, группах, расходах и платежах
    \item Разработать алгоритм оптимизации количества транзакций при расчёте взаиморасчётов между участниками
    \item Создать интуитивный пользовательский интерфейс, адаптированный для различных устройств
    \item Реализовать функциональность Progressive Web App
...
\begin{itemize}
    \item Управление пользователями и их аутентификация;
    \item Создание и администрирование групп участников;
    \item Учёт расходов с различными способами разделения сумм между участниками;
    \item Автоматический расчёт балансов и оптимизация схем взаиморасчётов;
    \item Регистрация выполненных платежей между участниками.
\end{itemize}
...
\begin{itemize}
    \item Регистрация новых пользователей с использованием email и пароля;
    \item Аутентификация пользователей.
\end{itemize}

\textbf{Управление группами:}
\begin{itemize}
    \item Создание групп;
    \item Генерация уникальных кодов приглашений;
    \item Присоединение пользователей к группам по коду приглашения.
\end{itemize}

\textbf{Учёт расходов:}
\begin{itemize}
    \item Добавление расхода с указанием описания, суммы, даты и плательщика;
    \item Выбор участников, между которыми разделяется расход;
    \item Поддержка различных способов разделения;
    \item Редактирование и удаление ранее добавленных расходов;
\end{itemize}

\textbf{Расчёт балансов между участниками:}
\begin{itemize}
    \item Автоматический расчёт баланса каждого участника;
    \item Оптимизация количества необходимых транзакций;
    \item Отображение схемы взаиморасчётов (кто кому и сколько должен).
\end{itemize}

\textbf{Регистрация платежей:}
\begin{itemize}
    \item Отметка о фактически выполненном переводе между участниками;
    \item Автоматический пересчёт балансов с учётом зарегистрированных платежей;
\end{itemize}
...
\textbf{React 19} \cite{banks2022react} -- современная JavaScript-библиотека для создания пользовательских интерфейсов на основе компонентного подхода. 

\textbf{Next.js 16} -- React-фреймворк с поддержкой серверного рендеринга. Используется для: API Routes для создания backend-функционала, file-based роутинга для удобной навигации.

\textbf{TypeScript 5} \cite{bierman2014typescript} -- типизированный JavaScript с поддержкой статической проверки типов. Обеспечивает раннее выявление ошибок на этапе разработки и улучшение поддерживаемости кода.

\textbf{Tailwind CSS 4} -- utility-first CSS фреймворк. Позволяет определять стили прямо в HTML-коде, используя предопределённые классы-утилиты.

\textbf{JavaScript} \cite{flanagan2020javascript} -- язык программирования, используемый для реализации логики приложения как на клиентской, так и на серверной стороне.

\subsection{Серверная часть}

\textbf{Node.js} \cite{cantelon2017node} -- серверная платформа на базе V8 JavaScript-движка. Обеспечивает единый язык для клиентской и серверной частей приложения, а также доступ к обширной экосистеме пакетов через менеджер npm.

\textbf{Next.js API Routes} -- встроенная возможность создания RESTful API \cite{tilkov2015rest, pautasso2008restful}. Файлы в директории \texttt{app/api/} автоматически становятся HTTP-эндпоинтами.

\textbf{SQLite} \cite{owens2006sqlite} -- встраиваемая реляционная СУБД. Используется библиотека Better-SQLite3 для синхронного взаимодействия с базой данных, что обеспечивает простоту работы.
...
База данных спроектирована в соответствии с принципами реляционной модели данных \cite{silberschatz2010database} и состоит из 8 таблиц.
...
реализация функции \texttt{createSession} приведена в листинге~\ref{lst:auth-create-session}.
...
пример функции установки cookie приведён в листинге~\ref{lst:auth-set-cookie}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/session_cookie.png}
  \caption{Cookie с идентификатором сессии}
\end{figure}
...
реализация генерации уникального кода показана в листинге~\ref{lst:invites-generate-code}.

Реализация генерации приглашения  приведена в листинге~\ref{lst:invites-create}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/invites_examples.png}
  \caption{Примеры приглашений}
\end{figure}

Примеры ссылки приглашения: \newline \url{localhost:3000/invite/558130fc7f7d34e9b4650578bc1fa6cb5dce7823c0e8d6c7}


\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{images/link_invite_accept.png}
  \caption{Пример перехода по ссылке приглашения}
\end{figure}

При повторном переходе по ссылке приглашения будет отображаться ошибка:
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{images/link_invite_accept_error.png}
  \caption{Ошибка перехода по ссылке приглашения}
\end{figure}
...
API-маршрут \texttt{api/groups/[id]/expenses/route.ts} принимает JSON формата указанного в листинге~\ref{lst:expenses-request}.


\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\textwidth]{images/expense_request.png}
  \caption{Запрос на создание расхода}
\end{figure}

Реализация в \texttt{api/groups/[id]/expenses/route.ts}:
фрагмент кода вставки расхода и разбивки приведён в листинге~\ref{lst:expenses-insert}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/expense_response.png}
  \caption{Ответ о создании расхода}
\end{figure}

Затем генерируем событие для обновления UI:
пример генерации события об изменении данных показан в листинге~\ref{lst:events-expense-created}.
...
API-маршрут \texttt{POST api/groups/[id]/payments/route.ts}:
полный обработчик создания платежа приведён в листинге~\ref{lst:payments-post}.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{images/payments_request.png}
  \caption{Запрос на создание платежа}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{images/payments_response.png}
  \caption{Ответ на создание платежа}
\end{figure}
...
Рреализация \texttt{EventBus} вынесены в листинг~\ref{lst:eventbus}.

\textbf{Использование в компоненте}:

В \texttt{GroupView.tsx} компонент подписывается на события группы:
пример использования \texttt{EventBus} на клиенте в компоненте \texttt{GroupView} приведён в листинге~\ref{lst:groupview-subscribe}.
...
В ходе выполнения курсовой работы была достигнута поставленная цель -- разработано функциональное веб-приложение TripF для управления групповыми расходами.

\textbf{Основные результаты работы:}

\begin{enumerate}
    \item Разработана архитектура веб-приложения на основе клиент-серверной модели с использованием React, Next.js и SQLite
    \item Разработан и протестирован жадный алгоритм минимизации количества транзакций при расчёте взаиморасчётов между участниками группы
    \item Создан адаптивный пользовательский интерфейс, работающий на различных устройствах
    \item Реализована функциональность Progressive Web App
    \item Проведено функциональное тестирование, подтвердившее корректность работы приложения
\end{enumerate}

\textbf{Дальнейшее развитие системы:}

Перспективными направлениями развития системы являются: интеграция с платёжными системами для автоматизации переводов, реализация поддержки множественных валют с автоматической конвертацией, внедрение машинного обучения для категоризации расходов.
\newpage
...
\subsection*{Ссылка на исходный код}

Исходный код приложения доступен по ссылке: \url{https://github.com/arpefly/tripf}

\subsection*{Листинги кода}

\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Создание сессии пользователя}, label={lst:auth-create-session}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Проверка актуальности сессии}, label={lst:auth-get-session}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Установка cookie с идентификатором сессии}, label={lst:auth-set-cookie}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Генерация уникального кода приглашения}, label={lst:invites-generate-code}]
const CODE_ALPHABET = 
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Создание приглашения в группу}, label={lst:invites-create}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Структура запроса на создание расхода}, label={lst:expenses-request}]
{
  "description": "desc",
  "amount": 0000,
  "paidBy": "user-uuid",
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Вставка расхода и разбивки по участникам}, label={lst:expenses-insert}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Генерация события о создании расхода}, label={lst:events-expense-created}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Обработчик создания платежа}, label={lst:payments-post}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Простая реализация EventBus}, label={lst:eventbus}]
...
\begin{lstlisting}[language=JavaScript, basicstyle=\small\ttfamily, caption={Подписка компонента GroupView на события группы}, label={lst:groupview-subscribe}]
useEffect(() => {
